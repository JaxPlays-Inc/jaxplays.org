...
This repository is the Local Theatre monorepo. It currently contains JaxPlays as the first city site and a shared Hugo theme that future city sites can reuse.
...
- `sites/jaxplays/content/news/` contains theatre news, site announcements, feature stories and community updates.
- `sites/jaxplays/content/reviews/` contains production reviews tied to specific shows and theatres.
- `sites/jaxplays/content/editorials/` contains opinion, context and longer-form commentary.
- `sites/jaxplays/content/auditions/` contains audition announcements and calls for performers.
...
- `sites/jaxplays/content/productions/` contains individual staged productions, organized by year.
...
- `sites/jaxplays/content/people/` contains profiles for actors, directors, designers, musicians, technicians, writers, stage managers and other theatre artists.
- `sites/jaxplays/content/theatres/` contains theatre company and producer profiles.
- `sites/jaxplays/content/venues/` contains physical performance spaces, addresses, maps and venue details.
...
- `sites/jaxplays/content/about/` for organization and project information.
- `/signup/` for newsletter subscription.
- `/support/` for donation and support paths.
- `sites/jaxplays/content/corporate-sponsors/` for sponsor records.

## Repository Structure

Important project areas:

- `sites/jaxplays/` is the JaxPlays Hugo site root.
- `sites/jaxplays/content/` is the editorial and directory content source.
- `sites/jaxplays/data/` contains structured data, including generated indexes.
- `sites/jaxplays/static/media/` contains headshots, posters, logos, photos, programs and other static assets.
- `themes/local-theatres/` contains the custom Hugo theme, layouts, partials, shortcodes, CSS and static theme assets.
...
That check regenerates `sites/jaxplays/data/generated/` and fails if the committed generated data is stale.
...
```bash
python3 scripts/update_people_roles.py --root sites/jaxplays --check
python3 scripts/update_people_roles.py --root sites/jaxplays --write
```

Run these from the repository root. If you are already inside `scripts/`, use:

```bash
python3 update_people_roles.py --root ../sites/jaxplays --check
python3 update_people_roles.py --root ../sites/jaxplays --write
```

The role updater edits real `sites/jaxplays/content/people/*.md` files. It is intentionally separate from the normal Hugo build path.
...
There is also a generated-data workflow that checks whether `sites/jaxplays/data/generated/` is current when relevant files change.

Do not treat a local Hugo build as a live publish. Deployment is controlled by the configured workflow and branch policy.

## Wikilinks

Content supports internal wikilinks through the custom Hugo partial at `themes/local-theatres/layouts/partials/content-wikilinks.html`.
...
The active theme lives in `themes/local-theatres/`. It includes:
